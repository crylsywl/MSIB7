  ...
  return (
    <div className="flex gap-4">
      {options.map((option) => (
        <label
          key={option.value}
          className={`flex items-center cursor-pointer font-sans hover:bg-[#A49AF6] hover:border-[#A49AF6] font-semibold gap-2 border-2 p-2 rounded-md  ${
        ...
        >
          ...
          <span
            className={` ${
              selected === option.value
                ? (<FontAwesomeIcon icon={faCheck} className="flex justify-center mb-0 mr-2" />)  // Border color when selected
                : (<FontAwesomeIcon icon={faCheck} className="flex justify-center mb-0 mr-2" />)  // Border color when not selected
            }`}
          >
            {selected === option.value && (
